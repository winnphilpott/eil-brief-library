...
1. **Reading and Understanding** Read through the paper, talk with PIs about any questions, note any technically dense and/or essential-to-know sections.
   ...
   b. **Brief Interview with the PI** Once you've read through, set up a short conversation with the PI. This is your chance to ask what they think the most important takeaway is, clear up anything the paper left you unsure about, and get a sense of how they'd explain the study to someone outside the field. It's also a good spot to pick up a quote for the press release later on — just make sure the PI signs off on the exact wording before it goes into anything.
2. **Research Highlight** Here is where we can get more into the mechanics of the study (while maintaining a general interest audience). We're moving past the intrigue of the press release and taking the time to teach people something. Think about how to communicate the mechanics of research in a digestable way. This is also the piece that ends up shaping everything else. Once you've worked out how to explain the study here, that framing should carry through to the snapshot and the press release instead of starting over each time. The standard build is two figures: an infographic on the first page that sets up the question or the research design, and a data viz on the second page that carries the finding. 
3. **Empirical Snapshot** Think about what is difficult to explain verbally, or a key point that is supported by some kind of data/empirical visualization.
...
The snapshot isn't automatically one of the highlight figures. It can be — if the page-two data viz is already the sharpest version of the point, use it — but the snapshot has to stand on its own without any surrounding text, so it often needs a different figure or a simpler cut of the same one.
