...
- **Optional Background diagram:** a conceptual figure (e.g. contrasting two approaches) may
  sit in the Background as a right-side `wrapfigure` with text flowing around it, instead of a
  centered minipage. Scope `\usepackage[none]{hyphenat}` (or a raised `\hyphenpenalty`) to that
